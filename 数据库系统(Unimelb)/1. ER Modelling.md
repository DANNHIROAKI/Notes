...
> > |      $\text{Concept}$      |              解释              |               备注               |
> > | :------------------------: | :----------------------------: | :------------------------------: |
> > |    $\text{Entity(Set)}$    | 真实世界中与其它对象区分的对象 |      单个实体由多个属性描述      |
> > |     $\text{Attribute}$     |       对于一个实体的描述       | 实体的属性中必有一个$\text{Key}$ |
> > | $\text{Relationship(Set)}$ |      两个/多个实体的联结       |  关系也可能有$\text{Attribute}$  |
> >
...
> >
> > > **1️⃣**$\text{Key/Cardinalitiy Constraints:}$ 关系分为$\text{1-1/1-M/M-M}$三种
> > >
...
> > >
> > > 1. 多对多<font color="red">(红色标出)</font> 
> > >
> > >    | $\text{Attributes}$ |  $\text{Employee}$   | $\text{Department}$  |
> > >    | :-----------------: | :------------------: | :------------------: |
> > >    |  $\text{One/Many}$  |    $\text{Many}$     |    $\text{Many}$     |
> > >    |        解释         | 员工可在几个部门工作 | 一个部门可有多个员工 |
> > >
> > >    <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240319175958694.png" alt="image-20240319175958694" style="zoom:59.5%;" />  
> > >
> > > 2. 一对多<font color="red">(Key Constraint红色标出)</font> 
> > >
> > >    | $\text{Attributes}$ | $\text{Employee/Manager}$ |  $\text{Department}$   |
> > >    | :-----------------: | :-----------------------: | :--------------------: |
> > >    |  $\text{One/Many}$  |       $\text{One}$        |     $\text{Many}$      |
> > >    |        解释         | 经理(员工)可管理多个部门  | 一个部门只能有一个经理 |
> > >
> > >    <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240517230200423.png" alt="image-20240517230200423" style="zoom:22%;" /> 
> > >
> > > 3. 一对一<font color="red">(Key Constraint红色标出)</font> 
> > >
> > >    | $\text{Attributes}$ | $\text{Employee/Manager}$  |  $\text{Department}$   |
> > >    | :-----------------: | :------------------------: | :--------------------: |
> > >    |  $\text{One/Many}$  |        $\text{One}$        |      $\text{One}$      |
> > >    |        解释         | 经理(员工)只能管理一个部门 | 一个部门只能有一个经理 |
> > >
> > >    <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240517230604729.png" alt="image-20240517230604729" style="zoom:22%;" /> 
> > >
> > > **2️⃣**$\text{Participation Constraints}$
> > >
...
> > > **3️⃣**两种限制的组合
> > >
> > > |      $\text{Type}$      | $\text{Participation}$ | $\text{Key Constraint}$ | $\text{E.g. Prof may teach\_\_lecture}$ |
> > > | :---------------------: | :--------------------: | :---------------------: | :-------------------------------------: |
> > > | $\text{Optional Many}$  |    $\text{Partial}$    |            ❌            |              $\text{0/N}$               |
> > > | $\text{Mandatory Many}$ |     $\text{Total}$     |            ❌            |              $\text{1/N}$               |
> > > |  $\text{Optional One}$  |    $\text{Partial}$    |            ✔️            |              $\text{0/1}$               |
> > > | $\text{Mandatory One}$  |     $\text{Total}$     |            ✔️            |               $\text{1}$                |
> >
...
> >    |    $\text{Entity}$    | 描述                                                         |
> >    | :-------------------: | ------------------------------------------------------------ |
> >    | $\text{Owner entity}$ | 弱实体所赖以存在的实体                                       |
> >    |   $\text{Weak key}$   | 依赖于强实体而存在的实体，==必须包含相应强实体的$\text{Key}$== |
> >
> > 2. $\text{Identifying/Weak Relationship:}$​ 连接强弱实体的关系，所有弱实体必定有一个弱关系
> >
> > **2️⃣**$\text{Chen's ER Diagram:}$ 弱实体/弱关系边框**加粗**，如下关系中员工家属$\text{(Dependent)}$是弱实体
> >
...
> > **1️⃣**$\text{Constraints}$ 
> >
> > |      $\text{Type}$      |               $\text{Chen's Vs. Crow's Foot}$                |
> > | :---------------------: | :----------------------------------------------------------: |
> > | $\text{Optional Many}$  | <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240518171505689.png" alt="image-20240518171505689" style="zoom: 35%;" /> |
> > | $\text{Mandatory Many}$ | <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240518171514714.png" alt="image-20240518171514714" style="zoom:35%;" /> |
> > |  $\text{Optional One}$  | <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240518171524051.png" alt="image-20240518171524051" style="zoom:35%;" /> |
> > | $\text{Mandatory One}$  | <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240518171531204.png" alt="image-20240518171531204" style="zoom:35%;" /> |
> >
...
> >
> > **3️⃣**$\text{Difference in Cardinalities Placement in 1-N Relationship}$
> >
> > |            $\text{Chen's}$的$\text{N}$在少的一侧             |          $\text{Crow's Foot}$的$\text{N}$在多的一侧          |
> > | :----------------------------------------------------------: | :----------------------------------------------------------: |
> > | <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240325234821273.png" alt="image-20240325234821273" style="zoom: 50%;" /> | <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240325234836617.png" alt="image-20240325234836617" style="zoom: 50%;" /> |
...
> > **1️⃣**$\text{Super Ksy:}$ 
> >
> > 1. 含义：$\text{A set of attributes that uniquely identifies each tuple}$
> >
> > 2. 特点：一个键值与一行一一对应，每个关系必定有一组属性组成超键(因为关系中每行必唯一)
> >
...
> > **2️⃣**$\text{Primary Key}$  
> >
> > 1. $\text{Key in a Relationship:}$ 一种特殊的$\text{Super Ksy}$，用最少的无冗余的信息，区分每一个$\text{Tuple}$ 
> >
> > 2. 类型
> >
...
> >    | $\text{Candidate}$ | 所有可能作为主键的属性(集) |     $\text{N/A}$     |   `login`   |
> >    | $\text{Composite}$ |     多个属性组成的主键     | 无单一属性主键时适用 | `sid+login` |
> >
> > 3. 示例
> >
...
>
> > **1️⃣**$\text{Integrity Constraint:}$​ 每个实例要遵守的规则，即合法实例
> >
> > 1. $\text{Referential Integrity:}$ 所有$\text{Foreign Key}$​(引用)必须有效
> > 2. $\text{Domain Constraint:}$ 比如一个属性被定义为整数，那么只能输入整数数据给它
> >
> > **2️⃣**示例
> >
> > <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240320032739617.png" alt="image-20240320032739617" style="zoom: 60%;" /> 
...
> > >
> > > <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240331211414035.png" alt="image-20240331211414035" style="zoom: 40%;" /> $\Longrightarrow$​<img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240331211449075.png" alt="image-20240331211449075" style="zoom: 38%;" /> 
> >
> > ### $\text{5.2.2. Binary Relationship}$
> >
> > > **1️⃣**$\text{N-N}$关系：$\text{Create an associative entity}$​​​ 
> > >
> > > 1. 规则
> > >
> > >    - 联结实体中要包含所有$\text{Descriptive Attributes}$
> > >    - 联结实体中要包含两边的$\text{PK}$作为$\text{FK}$
> > >
...
> > >
> > > 3. $\text{Crow's Foot}$​​示例
> > >
> > >    - $\text{ER Modelling}$ 
> > >
> > >      <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240518233905993.png" alt="image-20240518233905993" style="zoom:33%;" /> 
> > >
> > >    - $\text{Logical Design}$
> > >
> > >      <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240518233808020.png" alt="image-20240518233808020" style="zoom: 39%;" />
> > >
...
> > > |   $\text{Chen's}$    | $\text{PK from MANY side}\to{}\text{FK at ONE side}$ |
> > >
> > > 1. $\text{Crow's Foot}$示例：一个客户有多个账户
> > >
> > >    <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20240518235315127.png" alt="image-20240518235315127" style="zoom:50%;" /> 
> > >
> > > 2. $\text{Chen's}$​示例：$\text{ONE}$一侧不仅增加另一侧的主键，还要包括关系的描述属性
> > >
