...
> > > 3. 特殊情况：
> > >
> > >    - 若$\text{Student}$筛选剩下$\text{20\%}$再去$\text{Join}$，$\text{RF}$计算无需$\text{×20\%}$，永远保持原来的大小
> > >    - 因为$\text{Subject}$中还有未被筛选的那$\text{80\% Student}$数据
> > >
...
> >
> > > **1️⃣**$\text{Left-Deep Join Tree}$：执行多重$\text{Join}$的最优顺序，方便在$\text{RAM}$中并行处理
> > >
...
> > |     $\text{Hash Join}$      | $\text{3×NPages(Outer)+3×NPages(Inner)}$                     |
> > |        $\text{SMJ}$         | $\text{Sort(Outer)+ Sort(Inner)+ NPages(Outer)+ NPages(Inner)}$ |
> >
> > 1.  $\text{NPages(Outer)/(Memory-2)}$要==向上取整==
> >
> > 2.  $\begin{cases}\text{Sort(Outer)= 2×NumPasses×NPages(Outer)}\\\\\text{Sort(Inner)= 2×NumPasses×NPages(Inner)}\end{cases}$  
> >
> > 3.  最优解：$\begin{cases}\text{RAM大小: }\text{NPages(Outer)+2}\\\\\text{开销: Cost=NPages(Outer)+NPages(Inner)}\end{cases}$  
